...
class AbstractFighter():
    ...
    def update(self):
        ...
        if not block is None:
            self.rect.x += block.change_x
            self.change_y -= self.var['gravity'] * float(settingsManager.getSetting('gravityMultiplier'))/100.0
    ...
    def accel(self,_xFactor):
        if self.change_x > self.preferred_xspeed: #if we're going too fast
            ...
            self.change_x -= min(diff,_xFactor*float(settingsManager.getSetting('frictionMultiplier') if self.grounded else settingsManager.getSetting('aircontrolMultiplier')) / 100.0)
    ...
    def calcGrav(self, _multiplier=1):
        if self.change_y > self.preferred_yspeed:
            ...
            self.change_y -= min(diff, _multiplier*self.var['gravity'] * float(settingsManager.getSetting('gravityMultiplier'))/100.0) 
...
########################################################
    ...
    def applyKnockback(self, _damage, _kb, _kbg, _trajectory, _weightInfluence=1, _hitstunMultiplier=1, _baseHitstun=1, _hitlagMultiplier=1):
        ...
        p = float(self.damage)
        d = float(_damage)
        w = float(self.var['weight']) * float(settingsManager.getSetting('weightMultiplier')) / 100.0
        s = float(_kbg)
        b = float(_kb)
        ...
        additional_kb = .5*_baseHitstun*math.sqrt(abs(trajectory_vec[0])*(self.var['air_resistance']*float(settingsManager.getSetting('aircontrolMultiplier')) / 100.0)**2+abs(trajectory_vec[1])*(self.var['gravity']*float(settingsManager.getSetting('gravityMultiplier'))/100.0)**2)
        ...
        if hitstun_frames > 0.5:
            ...
            if not isinstance(self.current_action, baseActions.HitStun) or (self.current_action.last_frame-self.current_action.frame)/float(settingsManager.getSetting('hitstunMultiplier'))*100 <= hitstun_frames+15:
                self.setSpeed((total_kb+additional_kb)*di_multiplier, _trajectory)
                self.doHitStun(hitstun_frames*float(settingsManager.getSetting('hitstunMultiplier'))/100.0, _trajectory)
    ...
    def applyPushback(self, _kb, _trajectory, _hitlag):
        self.hitstop = math.floor(_hitlag*float(settingsManager.getSetting('hitlagMultiplier'))/100.0)
    ...
    def shieldDamage(self,_damage,_knockback,_hitlagMultiplier):
        if self.shield_integrity > 0:
            ...
            if _damage > 1:
                self.doAction('shieldStun')
                self.hitstop = math.floor((self.damage / 4.0 + 2.0)*_hitlagMultiplier*float(settingsManager.getSetting('frictionMultiplier'))/100.0)
                self.change_x = _knockback
                self.current_action.last_frame = math.floor(_damage*3/4.0*float(settingsManager.getSetting('shieldStunMultiplier'))/100.0)
        else:
            self.change_y = -15
            self.invincible = 20
            self.shield_integrity = 100
