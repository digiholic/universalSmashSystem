...
class DamageHitbox(Hitbox):
    ...
    def getOnHitSubactions(self, _other):
        from engine.subactions.behavior import applyHitstun,applyScaledKnockback,dealDamage,compensateResistance,applyHitstop
        
        hitstun_subaction = applyHitstun.applyHitstun(self.damage+self.charge_damage*self.charge, self.base_knockback+self.charge_base_knockback*self.charge, self.knockback_growth+self.charge_knockback_growth*self.charge, self.trajectory, self.weight_influence, self.base_hitstun, self.hitstun_multiplier)
        knockback_subaction = applyScaledKnockback.applyScaledKnockback(self.damage+self.charge_damage*self.charge, self.base_knockback+self.charge_base_knockback*self.charge, self.knockback_growth+self.charge_knockback_growth*self.charge, self.trajectory, self.weight_influence)
        damage_subaction = dealDamage.dealDamage(self.damage+self.charge_damage*self.charge)
        compensation_subaction = compensateResistance.compensateResistance(self.base_hitstun/2.0)
        hitstop_subaction = applyHitstop.applyHitstop(((self.damage+self.charge_damage*self.charge) / 3.0 + 3.0)*self.hitlag_multiplier, 0.0, 0.0)
    ...
    def onCollision(self,_other):
        if Hitbox.onCollision(self, _other):
            if self.article is None:
                self.owner.applyPushback(0, 0, ((self.damage+self.charge_damage*self.charge) / 3.0 + 3.0)*self.hitlag_multiplier)
    ...
    def compareTo(self, _other):
        ...
        if clank_state == -1:
            if self.article is None:
                self.owner.applyPushback((self.base_knockback+self.charge_base_knockback*self.charge)/5.0, self.getTrajectory()+180, ((self.damage+self.charge_damage*self.charge) / 3.0 + 3.0)*self.hitlag_multiplier + ((_other.damage+_other.charge_damage*_other.charge) / 3.0 + 3.0)*_other.hitlag_multiplier)
            return -1
...
class SakuraiAngleHitbox(DamageHitbox):
    ...
    def getOnHitSubactions(self, _other):
        ...
        from engine.subactions.behavior import applyHitstun,applyScaledKnockback,dealDamage,compensateResistance,applyHitstop
        hitstun_subaction = applyHitstun.applyHitstun(self.damage+self.charge_damage*self.charge, self.base_knockback+self.charge_base_knockback*self.charge, self.knockback_growth+self.charge_knockback_growth*self.charge, angle, self.weight_influence, self.base_hitstun, self.hitstun_multiplier)
        knockback_subaction = applyScaledKnockback.applyScaledKnockback(self.damage+self.charge_damage*self.charge, self.base_knockback+self.charge_base_knockback*self.charge, self.knockback_growth+self.charge_knockback_growth*self.charge, angle, self.weight_influence)
        damage_subaction = dealDamage.dealDamage(self.damage+self.charge_damage*self.charge)
        compensation_subaction = compensateResistance.compensateResistance(self.base_hitstun/2.0)
        hitstop_subaction = applyHitstop.applyHitstop(((self.damage+self.charge_damage*self.charge) / 3.0 + 3.0)*self.hitlag_multiplier, 0.0, 0.0)
        return [hitstun_subaction,damage_subaction,knockback_subaction,compensation_subaction,hitstop_subaction]+self.other_on_hit_actions
...
class AutolinkHitbox(DamageHitbox):
    ...
    def getOnHitSubactions(self, _other):
        ...
        from engine.subactions.behavior import applyHitstun,applyScaledKnockback,dealDamage,compensateResistance,applyHitstop
        hitstun_subaction = applyHitstun.applyHitstun(self.damage+self.charge_damage*self.charge, velocity*self.velocity_multiplier+self.base_knockback+self.charge_base_knockback*self.charge, self.knockback_growth+self.charge_knockback_growth*self.charge, angle, self.weight_influence, self.base_hitstun, self.hitstun_multiplier)
        knockback_subaction = applyScaledKnockback.applyScaledKnockback(self.damage+self.charge_damage*self.charge, velocity*self.velocity_multiplier+self.base_knockback+self.charge_base_knockback*self.charge, self.knockback_growth+self.charge_knockback_growth*self.charge, angle, self.weight_influence)
        damage_subaction = dealDamage.dealDamage(self.damage+self.charge_damage*self.charge)
        compensation_subaction = compensateResistance.compensateResistance(self.base_hitstun/2.0)
        hitstop_subaction = applyHitstop.applyHitstop(((self.damage+self.charge_damage*self.charge) / 3.0 + 3.0)*self.hitlag_multiplier, 0.0, 0.0)
        return [hitstun_subaction,damage_subaction,knockback_subaction,compensation_subaction,hitstop_subaction]+self.other_on_hit_actions
    ...
    def onCollision(self, _other):
        if Hitbox.onCollision(self, _other):
            if self.article is None:
                self.owner.applyPushback(0, 9, ((self.damage+self.charge_damage*self.charge) / 3.0 + 3.0)*self.hitlag_multiplier)
            self.owner.data_log.addToData('Damage Dealt',self.damage)
            _other.owner.trail_color = self.trail_color
            offset = random.randrange(0, 359)
            hit_intersection = self.rect.clip(_other.rect).center
        ...
        if clank_state == 1 and self.hp >= 0:
            if not isinstance(_other, InertHitbox) and (isinstance(_other, DamageHitbox) or isinstance(_other, GrabHitbox)) and self.owner != _other.owner and _other.article != None and _other.article.owner != self.owner and hasattr(_other.article, 'tags') and 'reflectable' in _other.article.tags:
                ...
                if hasattr(_other, 'damage') and hasattr(_other, 'shield_multiplier'):
                    ...
                    self.priority -= (_other.damage+_other.charge_damage*_other.charge)
        ...
        if Hitbox.compareTo(self, _other) == 1:
            if not isinstance(_other, InertHitbox) and (isinstance(_other, DamageHitbox) or isinstance(_other, GrabHitbox)) and self.owner != _other.owner and _other.article != None and _other.article.owner != self.owner and hasattr(_other.article, 'tags') and 'absorbable' in _other.article.tags:
                ...
                if hasattr(_other, 'damage'):
                    self.owner.dealDamage(-(_other.damage+_other.charge_damage*_other.charge)*self.damage_multiplier)
        return 0
        ...
        if clank_state == 1 and self.hp >= 0:
            if not isinstance(_other, InertHitbox) and (isinstance(_other, DamageHitbox) or isinstance(_other, GrabHitbox)) and not _other.ignore_shields and self.owner.lockHitbox(_other):
                ...
                if _other.article is None:
                    _other.owner.applyPushback((_other.base_knockback+_other.charge_base_knockback*_other.charge)/5.0, _other.getTrajectory()+180, ((_other.damage+_other.charge_damage*_other.charge) / 3.0 + 3.0)*_other.hitlag_multiplier)
            return 1
...
class InvulnerableHitbox(Hitbox):
    ...
    def compareTo(self, _other):
        if not isinstance(_other, InertHitbox) and isinstance(_other, DamageHitbox) and not _other.ignore_shields and self.owner.lockHitbox(_other):
            if self.article is None:
                self.owner.applyPushback((_other.base_knockback+_other.charge_base_knockback*_other.charge)/5.0, _other.getTrajectory(), ((_other.damage+_other.charge_damage*_other.charge) / 3.0 + 3.0)*_other.hitlag_multiplier)
            if _other.article is None:
                _other.owner.applyPushback((_other.base_knockback+_other.charge_base_knockback*_other.charge)/5.0, _other.getTrajectory()+180, ((_other.damage+_other.charge_damage*_other.charge) / 3.0 + 3.0)*_other.hitlag_multiplier)
            return 1
        return 0
