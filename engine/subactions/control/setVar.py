...
class setVar(SubAction):
    ...
    def execute(self, _action, _actor):
        ...
        if not self.attr =='': #If there's a variable to set
            if self.attr in hasattr(source, 'stats') and source.stats: #if it has a var dict, let's check it first
                if self.relative: source.stats[self.attr] += self.val
                else: source.stats[self.attr] = self.val
            elif self.attr in hasattr(source, 'variables') and source.variables:
